...
class TestWorkload:
    ...
    @pytest.mark.parametrize(
        "namespace, attributes, filters, expected",
        [
            (
                "default",
                ["spec.dns_policy"],
                {"spec.service_account": "default", "metadata.name": "web-0"},
                {"spec.dns_policy": ["ClusterFirst"]},
            ),
            ("rand", ["metadata.name", "metadata.labels"], {}, {}),
        ],
    )
    def test_select_pod(namespace, attributes, filters, expected):
        client = Client(contexts=["minikube"], namespace=namespace, attributes=attributes, filters=filters)
        wl = Workload(client)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "default",
                ["spec.replicas", "spec.progress_deadline_seconds"],
                {"metadata.name": "nginx-app"},
                {"spec.replicas": [3], "spec.progress_deadline_seconds": [600]},
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "default",
                ["spec.replicas", "spec.template.spec.scheduler_name"],
                {"metadata.name": "frontend"},
                {"spec.replicas": [3], "spec.template.spec.scheduler_name": ["default-scheduler"]},
            ),
            ("rand", ["metadata.name", "metadata.labels"], {"spec.replicas": 69}, {}),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "kube-system",
                ["spec.template.metadata.creation_timestamp"],
                {"metadata.name": "kube-proxy"},
                {"spec.template.metadata.creation_timestamp": [None]},
            ),
            ("rand", ["metadata.name", "metadata.labels"], {}, {}),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "default",
                ["spec.completions", "spec.parallelism", "spec.completion_mode"],
                {"metadata.name": "pi"},
                {"spec.completions": [1], "spec.parallelism": [1], "spec.completion_mode": ["NonIndexed"]},
            ),
            ("rand", ["metadata.name", "metadata.labels"], {}, {}),
        ],
    )
